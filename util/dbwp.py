...
class DBwp(DBLite):
    def __init__(self, *args, **kargv):
        ...
        self.re_site_tables = re.compile(r"\b("+"|".join(self.site_tables)+r")\b", re.IGNORECASE)
    ...
    def rm_blog(self, blog_id=None):
        ...
        for t in reversed(self.site_tables):
            t = prefix + t
            if t in self.tables:
                self.drop(t)
        self.execute("delete from blogs where id="+str(blog_id))
        if self.blog_id == blog_id:
            self.blog_id = None
